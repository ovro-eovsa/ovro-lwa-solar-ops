...
def run_calib(msfile, msfiles_cal=None, bcal_tables=None, do_selfcal=True, caltable_folder=None, logger_file=None,\
    ...
    if not os.path.isdir(outputvis):
        utils.swap_fastms_pols(msfile)
        utils.correct_fastms_amplitude_scale(msfile)
    ...
    try:
        ...
        sp.change_phasecenter(msfile_slfcaled)
...
def run_imager(msfile_slfcaled, imagedir_allch=None, ephem=None, nch_out=12):
    ...
    try:
        ...
        deconvolve.run_wsclean(msfile=msfile_slfcaled,imagename=helio_imagename,\
                                size=512, scale='1arcmin', niter=10, weight='briggs 0',\
                                fast_vis=True, predict=False)
        ...
        outfits_helio = utils.convert_to_heliocentric_coords(msfile_slfcaled, outfits,ephem=ephem)
        for str1 in ['-model.fits','-image.fits','-horizon-mask.fits']:
            os.system("rm -rf "+ helio_imagename+str1)
    ...
    try:
        ...
        if len(msfiles_slfcaled) == 0 or overwrite_ms:
            ...
            run_calib_partial = partial(run_calib, msfiles_cal=msfiles_cal, bcal_tables=bcal_tables, do_selfcal=do_selfcal,
                    logger_file=logger_file, caltable_folder=gaintable_folder, visdir_slfcaled=visdir_slfcaled, strong_source_subtraction=strong_source_subtraction)
            result = pool.map_async(run_calib_partial, msfiles)
            print ("called run_calib")
            timeout = 2000.
...
def run_pipeline(time_start=Time.now(), time_end=None, time_interval=10, delay_from_now=180., do_selfcal=True, strong_source_subtraction= False, 
        ...
        #logger_file='/fast/msurajit/realtime_pipeline/realtime_calib-imaging_fast_parallel.log',
        proc_dir = '/fast/bin.chen/realtime_pipeline/',
        save_dir = '/lustre/bin.chen/realtime_pipeline/',
        calib_dir = '/lustre/bin.chen/realtime_pipeline/caltables/',
...
if __name__=='__main__':
    ...
    parser.add_argument('--logger_prefix', default='solar_realtime_pipeline_fast', help='Prefix for logger file')
    ...
    try:
        run_pipeline(args.prefix, time_end=Time(args.end_time), time_interval=float(args.interval), nodes=args.nodes, delay_from_now=float(args.delay),
                     server=args.server, file_path=args.file_path,
                     ...
                     delete_working_fits=(not args.keep_working_fits), beam_fit_size=args.bmfit_sz, strong_source_subtraction=False)
